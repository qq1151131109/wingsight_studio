---
name: de-ai-zh
description: 去除中文文本中的 AI 写作痕迹，让文字更自然、更像真人书写。当用户要「去 AI 味」「写得像人一点」「别那么 AI」，或要改写 AI 味重的剧本正文/台词/旁白/宣发文案/策划案/摘要/文本卡正文时使用。识别并修复 24 种常见 AI 写作模式（夸大象征、宣传语、AI 词汇、破折号滥用、三段式、否定式排比、填充短语等）。
---
...
- 改写对象常见为**文本卡/剧本卡正文**：先 read_node 取全文，改写后经 canvas_ops update_node 的 body 写回原卡（不另建新卡）。
- 聊天里直接贴的段落：改写结果直接在回复里给出。
- 剧本正文涉及台词时**保留台词的口语特征**——台词本就该像人说话，只清理叙述部分的 AI 痕迹，不要把口语台词「修正」成书面语。
