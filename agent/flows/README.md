...
| `ref-research-brief.json` | 参考图调研·文字考据提纯 | 资产参考图调研文路：资产设定+网页正文 → 考据简报（视觉细节/时代特征/常见误用，每条带来源域名，400 字内，冲突写存疑）。纯原生链，载荷走 input_value JSON；简报喂 select 终选纠错配 + 落卡 data.researchBrief（写设定/出图设定供料） | `LANGFLOW_REF_BRIEF_FLOW_ID` | 无（组件名 LanguageModelComponent 注模型） |
