...
LAYOUT_SPECS: dict[str, str] = {
    ...
    "scene": (
        "横版 16:9 四格场景空间基准图，无人空镜：左侧约 40% 宽为空间主视图"
        "（中性勘景视角，完整呈现空间形制、布局与光线方向），右侧三个等宽"
        "面板依次为反打视角（从主视点对面回看，交代另一侧墙面、出入口与"
        "纵深）、四分之三斜角视角（从空间一角的斜向机位，同时看到相邻两面"
        "墙与地面的关系）、关键区域近景（最常发生戏的区域——主座/桌案/门窗"
        "/通道的陈设、材质与年代细节）。四个面板是同一空间同一套陈设，材质、"
        "光位与年代特征一致。**所有建筑构件与家具按真实尺度关系呈现**：门洞"
        "高度、台阶、栏杆与坐具的比例要经得起人体尺度推算，让后续分镜能"
        "判断空间大小。画面中不出现人物、动物、剧情行为或临时活动；这是"
        "空间设计与勘景参考图，不是正在发生剧情的电影剧照。"
    ),
    "prop": (
