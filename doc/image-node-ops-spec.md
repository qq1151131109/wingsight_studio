...
### P2 模板化图生图动作（机位 / 三视图 / 打光 / 人物质感）

共用「模板化重新生成」出卡管线（新卡+连线+对新卡 GENERATE_EVENT）：
...
- 生成本体不新写，GENERATE_EVENT 管线带出来的候选/补出/遥测全部继承；
  新卡首图无版本档可入（空卡首生成，正常）
- **机位（2026-09-04 升级为全交互弹窗，open-storyboard MultiAnglePanel 移植）**：
  相机球控（Canvas 2D 线框球 + CSS 3D 三相位相机模型 + 实时预览小屏 + 金色
  轨道线）+ 7 预设（自定义/鱼眼/倾斜/正面俯拍/正面仰拍/全景俯拍/背面）+
  水平 0-360/垂直 ±90 吸附滑杆 + 7 档景别（联动球心图缩放）+ 四种提示词模式
  （通用/单人/动作/多人）。提示词工程移植其 battle-tested 结构：角度区间
  防作弊条款（75-105° 真侧脸不许看向镜头 / 165-195° 真背面不许偷看）、
  禁止把相机设备画进画面、多人防克隆/防合并/防镜像——全文按 seedream 中文
  管线重写。竞品对比拍板：viedeo 球控漂亮但功能禁用（等 LoRA 部署）不搬，
  open-ai-canvas 132 行静态 SVG 表达力弱不搬
...
- **打光（2026-09-04 升级为全交互弹窗，open-storyboard LightingControlPanel
  移植）**：光位球控（方位/俯仰拖拽，光锥渐变+球心主体图）+ 主光源 6 方位钮 +
  亮度滑杆（三档措辞+微调档）+ 轮廓光开关 + 色温五点 + 预设卡网格（camera.py
  LIGHT_PRESETS 12 条单一事实源——**打光弹窗/导演台布光区/agent 速查表三处
  同源**，根治两套打光词表；卡底用竞品同款 CSS 渐变，不搬其 18MB 示例图）。
  方位→人话映射（不写「方位角 270°」原始角度、不给诱发画灯具的措辞）+
  「不要把灯具画进画面」条款是原实现核心手法，中文重写
...
## 7. 第二波：机位/打光全交互弹窗（2026-09-04，「按体验最好的来」拍板）

多视角 chips 弹窗与打光 chips 弹窗整体退役，替换为 open-storyboard-canvas
同款全交互弹窗（三家竞品对比：viedeo 球控最漂亮但功能处于禁用状态等 LoRA
部署、open-ai-canvas 静态 SVG 表达力弱——open-storyboard 完整且已上线，搬它）：

- **CameraAngleDialog + CameraSphereControl**：拖球定机位（经纬线随角度
  旋转的视差 + CSS 3D 三相位相机模型——正面背身带实时预览小屏/背面镜头/
  侧面薄身 + 金色轨道线 + 四向 ±12° 微调）；7 预设；水平/垂直吸附滑杆；
  7 档景别联动球心图缩放；通用/单人/动作/多人四模式拼词（多人防克隆/防合并/
  防镜像、75-105° 真侧脸与 165-195° 真背面防偷看条款，中文重写）
- **LightingDialog + LightingSphereControl**：光位球控（光锥渐变+球心主体图，
  补了原版缺失的 DPR 适配）+ 主光 6 方位钮 + 亮度 + 轮廓光 + 色温五点 +
  预设卡网格（CSS 渐变底，不搬 18MB 示例图）；方位→人话映射防「模型忽略
  原始角度」与「把灯具画进画面」两大坑
- **打光词表单一事实源根治**：camera.py 新增 LIGHT_PRESETS（12 条结构化：
  id/name/prompt），`/camera-vocab` 下发 lightPresets——打光弹窗预设卡、
  导演台布光区（lightHints 同源扁平视图）、agent 速查表三处同源；原先
  打光弹窗 8 预设与导演台 6 条 LIGHT_HINTS 两套词表的分裂消除
- 三视图/人物质感/全景环视维持 ImageTemplateDialog 轻量 chips 弹窗不动；
  出卡管线不变（新卡+连线+对新卡 GENERATE_EVENT 铁律）
- 回归 71/71（B 组迁移到机位弹窗流，C 组迁移到打光预设卡路径，新增
  B0a/B0b/C0 弹窗结构断言）；截图目视验收（球控渲染/轨道线像素检测/预设
  选中态）通过。实现：components/canvas/CameraSphereControl.tsx /
  LightingSphereControl.tsx / CameraAngleDialog.tsx / LightingDialog.tsx

## 8. 第三波：多功能模板五件 + ⌘K 命令面板（2026-09-04）

竞品全量盘点后的下一批（第一梯队）；盘点中确认小地图（MiniMap 主题化，
minimap pref 默认开）与视频帧条抽帧建卡（captureFrameAsNode：原生分辨率
seek→上传→图卡连线）**已存在**，不再重做。

- **多功能模板五件**（open-storyboard MultiFunctionPanel/promptTemplates
  中文版忠实移植，修其「左前三分之三」误译）：九宫格机位（3x3 多机位联系
  表，与机位弹窗互补——球控精调单角度，联系表一次看全部）/ 剧情推演
  （2x2 四拍叙事）/ 预测下一帧 / 回溯前帧 / 光影校正（调色 pass，与打光
  互补）。全部预设无关纯提示词模板走 ImageTemplateDialog 通用说明盒分支
  （panorama 分支泛化）；入口 = 工具条「多功能」下拉（NodeToolbar 屏幕空间
  层内相对锚定，无裁剪/无缩放补偿问题）+ 右键五项
- **⌘K 画布导航命令面板**：不另造组件——呼出既有 OutlinePanel 并升级为
  命令面板（自动聚焦搜索框、↑↓ 跨组循环高亮、Enter 定位选中、类型标签
  进过滤、渲染期 hi 复位避 React Compiler 规则）；⌘K capture 拦截防浏览器
