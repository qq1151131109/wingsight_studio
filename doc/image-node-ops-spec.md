...
- **三明治落位**（对齐竞品共识）：
  - 卡上角落簇（hover 既有）= 一秒钟能做完的轻动作
  - 右键菜单图片专属段 = 重动作入口的家
  - 灯箱 = 看图 + 已有的成品级动作（标注重绘/九宫格/设为主图/版本恢复），不再加新
...
**P3-1 放大/超分**：竞品现状——open-ai-canvas 的「放大」是**纯前端 canvas 插值**
（长边 4096 上限，非 AI），其「AI 超分」菜单**自己都标暂未实现**；唯一的真超分
= open-storyboard 的即梦 CLI（字节专有，不可搬）。**可搬的是 open-storyboard
的两遍精修管线**（toolProcessor.ts runHdPipeline：第一遍出黑白结构线稿做结构锚，
第二遍原图+线稿精修；失败降级单遍）——本质是普通图生图，走现有 DMX 编辑通道
即可。前置调研：DMX 有无真实超分通道（models.py 探针实测）；有则加目录项，
没有则评估两遍管线，**本地插值假超分不做**（违反「不降级」铁律）。
落地形态 = agent 直连 DMX 通道 + 异步 job（POST /storyboard/images 的 jobId
轮询范式，10 分钟代理上限）——**不走 Langflow**（架构铁律：Langflow 管 LLM
文本生成，出图类是 agent 直连模型通道）。

**P3-2 扩图 outpaint**：open-storyboard 横/竖/均衡三模板（prompt + 编辑模型画布
扩展）。前置调研：gpt-image / seedream edit 通道的画布扩展能力（doc/dmxapi-*.md
实测）。落地形态同 P3-1（直连 + 异步 job，不走 Langflow）。

**P3-3 全景图**：open-storyboard 有全套（panoramaPrompt.ts 强制 2:1 等距柱状
职责化 prompt + panoramaNormalize 本地 canvas 归一化 + 全景查看节点；smart 模式
合成白色 2:1 参考图诱导 i2i 输出全景比例），open-ai-canvas 仅 three.js 查看节点。
落点：场景卡「环视」动作。前置调研：**DMX 模型 2:1/4:1 画幅支持**（models.py
aspects 现收主流 6 档、21:9 是收口，2:1 大概率要加档并实测像素上限）；
three.js 查看器是纯前端新依赖，评估包体。两项调研后再补详细 spec。

两项以上做完调研再补详细 spec，本档只占位。
...
| 项 | 状态判定 |
|---|---|
| P0-1 | 有图图片卡角落簇见 下载/复制图片，点击即生效，遥测落库 |
| P0-2 | 图片卡右键见专属段；空卡/他类型卡不出现 |
| P1-1 | 裁剪→原位替换+版本+1，比例预设与手柄可用，撤销可回滚 |
| P1-2 | 右键切自由后可任意拉伸，切回锁定按图比例回弹 |
| P1-3 | 多选图片/分镜表卡 → 合成大图下载，帧编号与间距正确 |
| P2 | 四个动作各自出 预设弹窗→新卡连线（源卡图不被覆盖），事件载荷正确，无画风时被闸拦截并弹画风窗 |
| P3 | 调研结论落档后另立验收 |

顺序建议：P0 一次提完（半天级）→ P1-1 → P2（四动作一批）→ P1-2/P1-3 顺手 →
P3 调研。
