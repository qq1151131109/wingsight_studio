...
## 四之七、第三轮多场景实测 + 上下文审计（2026-09-12，commit 90d543c）

**上下文审计发现一个真 bug**：缺口台账的 covered/missing 会互相矛盾——终选每轮只看本轮候选，第 2 轮会把第 1 轮已覆盖的维度换个说法再报进 missing，planner 同时收到矛盾信号。修：累积时双向子串匹配修剪（自由文本维度，变体视为同维）。C11 锁行为。

**五新场景**：
| 场景 | 结果 | 发现 |
|---|---|---|
| A 架空虚构（玄夜王朝，factuality=fiction） | 3 采纳（故宫藻井/龙纹元素） | **调研链路无题材意识**：照跑史实文路、终选按真实年代判；planner 自发映射「庆余年/九州缥缈录 剧照」（合理策略）。记录为已知行为 |
| B 民国（1930s 上海旗袍店） | 2 采纳（旗袍实物照） | 法租界店门面稀缺，终选精准报「无可用画面参考」×3 轮 |
| C 西式（维多利亚伦敦） | 3 采纳（Reddit 1880s 老照片×2） | **中文检索西式内容完全可行**；1 张仿建小镇攻略图混入（画面优先 vs 来源判据的张力的可接受案例） |
| D 手填词（武则天） | 3 采纳（剧照） | 路径正常：首轮手填、不足仍 planner 补搜 |
| E 重跑语义（唐代烛台） | 重跑后 37 行 = 新 34 + 旧采纳 3 保留，采纳 3+3 | **完全正确**：跨轮去重、已采纳占坑、未采纳清层 |

**流程成本观察（B 档判停的实测代价）**：缺口追补几乎总是触发（终选几乎总会报 missing）——常态从「推荐够即停」变成「推荐够 + 追 1-2 轮」，词数与耗时约 ×2（D：3 词手填 → 实跑 6 词 72s）。这是 B 档的设计代价（换「对路的图」），上限受 `_GAP_CHASE_ROUNDS=2` 兜住；若观察下来成本不可接受，可把追补条件收紧为「missing 数 ≤2」（只追精确缺口）或下调追补轮数。

**另补**：单卡出图（资产卡直点「AI 出图」）走独立轮询循环，此前没接 refGap 提示——已补（与 pollShotImageJob 同口径，只提示一次）。

## 四之八、检索质量看图专项：壁画/人俑与古画（2026-09-12，commit e6245c9）

用户问「现在检索质量怎么样、有没有剧照、之前壁画/人俑的问题还在吗」。针对最易被淹没的
唐代女性人物/服饰跑专项实测，并**把采纳图下载下来直接看**（标题判断不可靠）：

- **壁画/人俑已被治住**：43 张候选 0 壁画 0 人俑——planner 禁区（馆藏/文物/出土/复原图
  不进图词）+ 终选拒博物馆展陈（画面类）双闸生效。
- **但发现同类新形态：传世古画**。《簪花仕女图》被 character 采纳（看图确认绢本古画）。
  钻空子的方式：①画作以「拍下来的画册/展览图」出现，「拍出来的还是渲染出来的」判据失效；
  ②画面类拒收清单没有点名绘画作品。同池的《长安十二时辰》定妆照排在古画之后没被采纳。
- **形制类的理想样本确认**：装束复原团队真人复原实拍被 costume 采纳——形制准确 + 摄影质感，
  正是「考据形制」与「生图可用」的交集。

**修复（分级，不一刀切）**：画面类（scene/character）拒绘画作品（古画/壁画/绢画/工笔/
线描/画像砖/俑像），优先序 剧照/定妆照 > 复原真人实拍 > 纪实照片；形制类（costume/prop）
画作降级参考（无摄影时代的合法形制证据），只在池内无实物/复原/剧照时可选且必须标注
「源自画作（有画风差异）」。

**复测**：唐代侍女采纳从《唐宫仕女图》古画 ×3 → 《妖猫传》摄影专访 + 《长安十二时辰》
服饰解析 + 妖猫传杨贵妃剧照，全部为影视摄影内容。

**方法论**：判断「图的质量」必须看图，标题与来源域只够粗筛——本轮三张关键图（古画/复原
实拍/剧照）的标题都看不出实际内容。

## 四之九、调研产物的使用链路验证（2026-09-12，commit 2634f6f）

用户问「调研结果有没有正确用到生图里、生图提示词有没有准确描述参考图怎么用」。验证方式：临时项目（资产 + 考据简报主体条目 + 2 张调研参考卡连线 refSource=research）走产品同款通道 `start_storyboard_image_job` **真出 1 图**，读 imagejobs 落库的 `finalPrompt`
（提示词渲染的最终事实，任何界面看不到的都在这）。

**结论：使用链路是通的**——
1. 参考图进生成：职责段按位次列出全部 2 张（参考图1（佛光寺大殿内景）/参考图2（敦煌藻井图案））。
2. 考据简报全文进提示词：含「常见误用：明清式蟠龙藻井、琉璃瓦彩画混入唐代」这类**判错依据**
   （终选看图用它判形制、出图用它避坑，同一份事实两处消费）。
3. 每张参考图有**专属职责描述**（juben build_reference_usage 范式）：「作为考据参考：只用于
   锁定形制、材质、结构与年代真实特征，在本画面中还原这些特征；**不继承该图的背景、展陈
   环境、拍摄角度与光影**」——按标签类型分流（character/scene/prop/costume/image/reference/
   shotref 各有职责文案），调研参考卡（refSource=research → type=reference）命中考据参考职责。

**发现并修一个瑕疵**：最终提示词渲染成「全局视觉风格（…务必遵循）：**全局视觉风格：**写实
影视质感」双重前缀——前端 4 处与聊天工具都先带「全局视觉风格：」前缀，flow 模板的
`{visual_notes}` 槽位外层还有同名标题。修：`_generate_single_image`（所有出图路径汇聚点）
构建载荷前剥掉开头一次前缀；**刻意放在 compose 之后**（指令合成要按该前缀提取画风行）。

回归：test_asset_research_brief 150 项（新增 I3 三断言）+ test_ref_select 83 + test_ref_report 113。
