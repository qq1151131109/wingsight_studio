...
- 文本模型平台化多目录（剧本/分镜表/拆解/提示词优化等 LLM 文字生成）：同文件唯一事实源（TEXT_MODELS，每条目带 `provider` 字段=langflow 一等命名平台），`GET /models/text` 下发。平台由 fork 扩展包 `langflow/src/bundles/platforms/` 声明（加平台只改 extension.json+重启 langflow，见 bundles/platforms/README.md；setup-langflow.sh 从 .env.local 自动种子平台变量）：glm-5.3-flash（出厂默认）/glm-5.3 → BigModel=智谱官方（BIGMODEL_BASE_URL/BIGMODEL_API_KEY）；deepseek-v4-flash/v4-pro/v4-flash-vision-exp → DeepSeek 平台（DEEPSEEK_BASE_URL/DEEPSEEK_API_KEY；本部署 BASE_URL 指向智谱 coding 网关，无官方键，改变量即切）；gpt-5.6-luna/gemini-3.7-flash/claude-sonnet-5 → DMX（DMX_BASE_URL/DMX_API_KEY）。旧 "OpenAI"/"OpenAI Compatible" 劫持命名及 OPENAI_*/OPENAI_COMPATIBLE_* 全局变量已下线。卡片级覆盖 `data.textModel`——chip 在**选中剧本/分镜表/文本卡后的下方输入条**（空=flow 出厂 glm-5.3-flash；剧本卡「拆分镜表」会把选择带给新建分镜表卡），端点透传：分镜表生成 `model` / 拆解 `text_model` / 提示词优化 `model` / 文本撰写 `model`（`/text/rewrite`，文本卡/剧本卡「撰写」直连管线——结果预览采用才覆盖正文，不再经聊天主循环），非法 id 400 明报。注入走 tweaks **组件名**：skills 经 `models.text_model_tweaks()` 同时注 model_name+provider（通道路由，不烧节点 id；提示词优化已拆双 flow——扩写态走原生链同此，看图反推为固定 gemini 视觉不开放切换）；聊天改分镜表不经此（聊天主循环走 `AGENT_MODEL`=glm-5.3-flash+AGENT_VISION_ENABLED=1）
