...
- 文本模型切换（剧本/分镜表/拆解/提示词优化等 LLM 文字生成）：同文件唯一事实源（TEXT_MODELS，DMX chat 探针验证），`GET /models/text` 下发。卡片级覆盖 `data.textModel`——chip 在**选中剧本/分镜表卡后的下方输入条**（与出图模型 chip 同范式，空=flow 出厂 deepseek-v4-flash；剧本卡「拆分镜表」会把选择带给新建分镜表卡），端点透传：分镜表生成 `model` / 拆解 `text_model` / 提示词优化 `model`，非法 id 400 明报。注入走 tweaks **组件名**（`LanguageModelComponent` 的 model_name 覆盖字段，不烧节点 id；提示词优化已拆双 flow——扩写态走原生链同此，看图反推为固定 gemini 视觉不开放切换）；聊天撰写/修改不经此（聊天主循环走 `AGENT_MODEL`）
