 ...
 *  **服饰紧跟角色**（2026-09-11 用户口径）：服饰不是与场景/道具平行的「又一堆
 *  资产」，它有主——造型计划把服饰绑在具体角色上（角色卡 looks[i].costume →
 *  costumeId），造型图出图的「参考图2=服饰结构图」也是为那个角色锁形制；画布上
 *  另有 服饰→造型卡 连线表达这层关系。排成「角色 服饰 场景 道具」后这一对相邻、
 *  场景/道具（环境与物件家族）退到外侧。改顺序时**两处一起改**——这里与
 *  nodes.tsx 拆解链的 KIND_ORDER（那边还有「造型图」框贴角色组右缘的预留位，
 *  服饰排在角色之后正好落在预留位右侧：角色|造型图|服饰|场景|道具，两条派生边
 *  都不横穿别的组框） */
 ...
 *  在现有内容下方开一条「资产带」——角色/服饰/场景/道具各一组框、组内
